...
diag2Json :: EL'Diagnostic -> ArgsPack
diag2Json
  ( EL'Diagnostic
    ...
    ArgsPack [] $
      odFromList
        [ ( AttrByName "range",
                    ...
                    [ ( AttrByName "start",
                        EdhArgsPack
                          ( ArgsPack [] $
                              odFromList
                                [ ( AttrByName "line",
                                    EdhDecimal $ fromIntegral $ max 0 start'line
                                  ),
                                  ( AttrByName "character",
                                    EdhDecimal $ fromIntegral $ max 0 start'char
                                  )
                                ]
                          )
                      ),
                      ( AttrByName "end",
                        EdhArgsPack
                          ( ArgsPack [] $
                              odFromList
                                [ ( AttrByName "line",
                                    EdhDecimal $ fromIntegral $ max 0 end'line
                                  ),
                                  ( AttrByName "character",
                                    EdhDecimal $ fromIntegral $ max 0 end'char
                                  )
                                ]
                          )
                      )
          ...
          (AttrByName "severity", EdhDecimal $ fromIntegral severity),
          ( AttrByName "code",
            case code of
              Left !i -> EdhDecimal $ fromIntegral i
              Right !s -> EdhString s
          ),

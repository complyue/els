...
blockSymbols !stmts = concat $ stmtSymbols <$> stmts
  where
    ...
    stmtSymbols (StmtSrc (PerceiveStmt _eosOnly !x !body) _) =
    ...
    exprSymbols !doc _full'span (IncExprExpr !src) = exprSymbols' doc src
...
blockFoldRngs (stmt1 : more'stmts) = foldGap stmt1 more'stmts
  where
    ...
    stmtFoldRngs (StmtSrc (PerceiveStmt _eosOnly !x !body) _) =
    ...
    exprFoldRngs _full'span (IncExprExpr !src) = exprFoldRngs' src
